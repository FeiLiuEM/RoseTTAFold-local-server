...
while True:
    time.sleep(60) #时间间隔，单位是秒，每隔5秒运行一次
    ...
    if os.path.exists("/home/math/DATA/RoseTTAFold/running/models/model_00_pred.pdb") == True:     # check if it is running
    ...
    if apply.values.tolist() != []:
        ...
        if os.path.exists("/home/math/DATA/RoseTTAFold/running/test.fa") == False:
            ...
            if result.loc[result['Amino_acid_sequence'] == apply.values.tolist()[0][2]].values.tolist() != []:
                ...
                same[1]=str(apply.values.tolist()[0][1])
               ...
               #将新数据合并到结果csv中
                ...
                os.system("cd /home/math/DATA/RoseTTAFold;./apply_rf2.sh")   #运行计算程序
                notification = "正在运行 "+str(apply.values.tolist()[0][1])
                print(notification)
